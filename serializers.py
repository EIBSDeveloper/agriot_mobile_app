...
class VendorPurchasePayablesOutstandingSerializer(serializers.Serializer):
    date = serializers.DateField(
        required=False,
        format='%d-%m-%Y',
        input_formats=['%d-%m-%Y'],
        help_text="Payment date in dd-mm-yyyy format",
        allow_null=True,
    )
    expense_id = serializers.IntegerField(required=True, help_text="ID of the purchase expense")
    payment_amount = serializers.DecimalField(max_digits=12, decimal_places=2, required=True, help_text="Payment amount")
    description = serializers.CharField(required=False, allow_blank=True, help_text="Optional description")
    documents = serializers.ListField(
        child=serializers.DictField(child=serializers.CharField()),
        required=False,
        help_text="List of document objects with fields 'file_type' (int) and 'document' (base64 string)"
    )
